 ...
 *
 * Honesty contract:
 *   - Every math attempt is a real function generated by the LLM and executed
 *     against the real acceptance test in a sandbox. Nothing is invented.
 *     A failed attempt is still a real data point.
...
// sandboxed Function context. Uses the same executeTestSuite as the sandbox.
...
// --- Stub generator (honest fallback) -----------------------------------------
// Generates a function with the correct signature that will fail the acceptance
// test honestly. Never fabricates a passing answer.

function generateStub(toolName: string, problem: HardMathProblem): string {
  const stubs: Record<string, string> = {
    collatzTotalStopping:
      `export function collatzTotalStopping(N) { return { maxSteps: 0, argmax: 1 }; }`,
    maxPrimeGap:
      `export function maxPrimeGap(N) { return { gap: 0, primes: [] }; }`,
    zetaZeroCount:
      `export function zetaZeroCount(T) { return { N: 0, lastImPart: 0 }; }`,
    factorialDigitChain:
      `export function factorialDigitChain(n) { return { cycle: [], fixedPoint: null }; }`,
    prothPrimality:
      `export function prothPrimality(n) { return []; }`,
    goldbachCheck:
      `export function goldbachCheck(N) { return { verified: false, minEven: 4, maxEven: N, totalPairs: 0 }; }`,
    collatzVerifiedRange:
      `export function collatzVerifiedRange(N) { return { verified: false, minN: 1, maxN: N, maxTotalSteps: 0 }; }`,
    riemannSearch:
      `export function riemannSearch(T) { return { candidates: [], criticalCount: 0 }; }`,
    bealSearch:
      `export function bealSearch(B) { return { counterexamples: [], testedQuadruples: 0 }; }`,
  };
  return (
    stubs[toolName] ??
    `export function ${toolName}(...args) { return null; }`
  );
}
...
export async function runMathCycle(): Promise<MathCycle> {
  ...
  if (candidate) {
    ...
    // LLM unavailable — fall back to stub (which will fail honestly).
    sourceCode = generateStub(toolName, problem);
    const result = runAcceptanceTest(sourceCode, toolName, problem.acceptanceTest);
    passed = result.passed;
    score = result.score;
    failureReason = failureReason ?? result.error ?? 'llm unavailable + stub failed';
  }
